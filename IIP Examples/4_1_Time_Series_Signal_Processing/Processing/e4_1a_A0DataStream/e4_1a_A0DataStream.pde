...
void draw() {
  ...
  stroke(0);
  beginShape();
  for (int n=0; n<sensorHist[0].length; n++) {
    float h = sensorHist[0][n];
    vertex(n, h);
  }
  endShape();
  //lineGraph(float[] data, float lowerbound, float upperbound, float x, float y, float width, float height, int _index)  
  //e.g., lineGraph(sensorHist[0], 0, height, 0, 0, width, height, 0); //toolfunction for drawing sensor data stream
}
